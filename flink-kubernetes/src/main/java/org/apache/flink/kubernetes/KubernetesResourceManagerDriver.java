...
public class KubernetesResourceManagerDriver extends AbstractResourceManagerDriver<KubernetesWorkerNode> {
	...
	protected void initializeInternal() throws Exception {
		kubeClientOpt = Optional.of(kubeClientFactory.fromConfiguration(
			flinkConfig,
			getIoExecutor()));
	...
	public CompletableFuture<KubernetesWorkerNode> requestResource(TaskExecutorProcessSpec taskExecutorProcessSpec) {
		...
		final CompletableFuture<Void> createPodFuture =
			podCreationCoolDown.thenCompose((ignore) -> getKubeClient().createTaskManagerPod(
				taskManagerPod));
	...
	private void recoverWorkerNodesFromPreviousAttempts() throws ResourceManagerException {
		List<KubernetesPod> podList = getKubeClient().getPodsWithLabels(KubernetesUtils.getTaskManagerLabels(
			clusterId));
		...
		getMainThreadExecutor().execute(() -> {
			for (KubernetesPod pod : pods) {
				if (pod.isTerminated()) {
					...
					final CompletableFuture<KubernetesWorkerNode> requestResourceFuture = requestResourceFutures
						.remove(podName);
					if (requestResourceFuture != null) {
						log.warn("Pod {} is terminated before receiving the ADDED event.", podName);
						requestResourceFuture.completeExceptionally(new FlinkException(
							"Pod is terminated."));
					}
	...
	public class PodCallbackHandlerImpl implements FlinkKubeClient.WatchCallbackHandler<KubernetesPod> {
		...
		public void reWatchPods() {
			podsWatchOpt = Optional.of(getKubeClient().watchPodsAndDoCallback(
